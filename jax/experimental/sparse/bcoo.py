...
class BCOO(JAXSparse):
  ...
  def from_scipy_sparse(cls, mat, *, index_dtype=None, n_dense=0, n_batch=0):
    ...
    # Check if row-major indices are sorted in non-decreasing order.
    is_sorted = True
    if any(jnp.diff(mat.row) < 0):
      is_sorted = False
    else:
      _, indice_segments = jnp.unique(mat.row, return_index=True)
      for i in range(indice_segments.shape[0] - 1):
        if any(jnp.diff(mat.col[indice_segments[i]:indice_segments[i+1]]) < 0):
          is_sorted = False
          break

    indices = jnp.column_stack((mat.row, mat.col)).astype(
        index_dtype or jnp.int32)
    return cls((data, indices), shape=mat.shape, indices_sorted=is_sorted)
